  ...
  return (
    <AdminLayout>
      <div className="p-4 sm:p-6 max-w-4xl mx-auto">
        ...
        <div className="portal-panel portal-panel-blue rounded-2xl p-5 mb-4">
        ...
        <div className="portal-panel portal-panel-purple rounded-2xl p-5 mb-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
            {DOCUMENT_THEMES.map(t => (
              <button key={t.id} type="button" onClick={() => updateSettings({ defaultTheme: t.id as DocumentTheme })} className="portal-panel portal-panel-slate p-3 rounded-xl text-left" style={{ borderColor: settings.defaultTheme === t.id ? t.accent : '#e2e8f0' }}>
        ...
        <div className="portal-panel portal-panel-cyan rounded-2xl p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            {BACKGROUND_TEMPLATES.map(t => (
              <button key={t.id} type="button" onClick={() => updateSettings({ defaultBackgroundTemplate: t.id as BackgroundTemplate })} className="portal-panel portal-panel-slate rounded-xl overflow-hidden border-2" style={{ borderColor: settings.defaultBackgroundTemplate === t.id ? '#1769AA' : '#e2e8f0' }}>
          ...
          <button type="button" onClick={() => bgRef.current?.click()} className="btn-3d btn-3d-primary btn-3d-scifi px-4 py-2 rounded-xl text-sm">Use as default background (upload)</button>
...
function LogoSlot({ label, value, onUpload, onRemove }: { label: string; value: string; onUpload: () => void; onRemove: () => void }) {
  return (
    <div className="portal-panel portal-panel-slate rounded-xl p-3">
      ...
      <div className="flex gap-2">
        <button type="button" onClick={onUpload} className="btn-3d-ghost text-xs font-semibold px-2 py-1 rounded-lg">{value ? 'Replace' : 'Upload'}</button>
